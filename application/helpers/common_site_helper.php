...
function saveFields(   $section,  $cont,$nm,$site_lang) {
    extract($cont);
    //pr($cont);
    $CI = get_instance();
    $cpt =count($nm);
...
function get_furniture_cat($id)
{
    global $CI;
    $CI = get_instance();
    $row = $CI->master->getRow('furniture_cat', array('id' => $id));
    return ($row->title);
}
...
function get_sub_cat($id)
{
    global $CI;
    $CI = get_instance();
    $row = $CI->master->getRow('sub_cat', array('id' => $id));
    return ($row->title);
}
