<?php if ($this->uri->segment(3) == 'manage'): ?>
    ...
    <div class="row margin-bottom-10">
        ...
        <div class="col-md-6 text-right">
            <a href="<?= site_url(ADMIN . '/sub_categories'); ?>" class="btn btn-lg btn-default"><i class="fa fa-arrow-left"></i> Back</a>
        </div>
    </div>
    <div>
        <hr>
        <div class="row col-md-12">
            <form action="" name="frmFaq" role="form" class="form-horizontal" method="post" enctype="multipart/form-data">
                <div class="form-group">
                    ...
                    <div class="col-md-6">
                                        <label for="cat_id" class="control-label">Furniture Category<span class="symbol required">*</span></label>
                                        <select name="cat_id" id="cat_id" class="form-control" required>
                                            <option value=''>-- Select --</option>
                                            <?php foreach ($cats as $index => $c) { ?>
                                                <option value="<?= $c->id ?>" <?= ($row->cat_id == $c->id) ? 'selected' : '' ?>> <?= $c->title ?></option>
                                            <?php
                                            }
                                            ?>
                                        </select>
    ...
    <?php else: ?>
        ...
        <div class="row margin-bottom-10">
            <div class="col-md-6">
                <h2 class="no-margin"><i class="entypo-list"></i> Furniture Sub_Categories</h2>
            </div>
            <div class="col-md-6 text-right">
                <a href="<?= site_url(ADMIN . '/sub_categories/manage'); ?>" class="btn btn-lg btn-primary"><i class="fa fa-plus-circle"></i> Add New</a>
            </div>
        </div>
                ...
                <?php if (count($rows) > 0): $count = 0; ?>
                    <?php foreach ($rows as $row): ?>
                        <tr class="odd gradeX">
                            ...
                            <td><b><?= $row->title; ?></b></td>               
                            <td><b><?= $row->cat_id; ?></b></td>       
                            <td class="text-center"><?= get_member_active_status($row->status); ?></td>
                            <td class="text-center">
                                <a href="<?= site_url(ADMIN.'/sub_categories/manage/'.$row->id); ?>">Edit</a> |
                                <a href="<?= site_url(ADMIN.'/sub_categories/delete/'.$row->id); ?>" onclick="return confirm('Your are about to delete this FAQ. Press OK to continue.');">Delete</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
